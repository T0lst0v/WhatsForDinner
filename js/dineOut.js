...
function getNearbyPlaces(position) {
    // api request
    let request = {
        location: position, rankBy: google.maps.places.RankBy.DISTANCE, keyword: currentCuisine,
    };
...
function displayRestaurants() {
    ...
    searchResults
        ...
        .forEach((result) => {
            ulRestaurants.insertAdjacentHTML("beforeend", `
            <li>
                <a href='javascript:void(0);' onclick="updateRestaurantDisplay('${result.name}')">
                    ${result.name} (${result.rating} \u272e)</a>
            </li>`); // javascript:void(0) to disable page going to top on link click
        });
...
function moveMap(restaurant) {
    ...
    marker = new google.maps.Marker({
        position: restaurant.geometry.location, map: map, title: restaurant.name, animation: google.maps.Animation.DROP,
    });
...
function displayRestaurantInfo(result, status) {
    ...
    divPhotos.insertAdjacentHTML('beforeend',`
